...
# ===========================================================================
# SAP integration benchmarks (require credentials)
# ===========================================================================
...
class TestCapabilitiesSAPBenchmark:
    ...
    LLM guidance in real SAP scenarios. They require SAP WebGUI credentials
    and use the MCP client fixture.

    Run with: pytest unittests/test_benchmark_capabilities.py -k "SAP" -v -s
    """

    @pytest.fixture
    def _require_sap_creds(self) -> None:
        from unittests.conftest import has_sap_webgui_creds

        if not has_sap_webgui_creds():
            pytest.skip("SAP WebGUI credentials not configured")

    async def test_capabilities_response_over_mcp(
        self, _require_sap_creds: None, sap_mcp_client: "ClientSession"
    ) -> None:
        ...
        print(f"Knowledge: {len(data.get('sap_knowledge', '')):>8,} chars")
        print(f"{'='*60}")

    async def test_login_still_recommends_capabilities(
        self, _require_sap_creds: None, sap_mcp_client: "ClientSession"
    ) -> None:
    ...
    async def test_shortcuts_section_actionable(
        self, _require_sap_creds: None, sap_mcp_client: "ClientSession"
    ) -> None:
    ...
    async def test_multi_session_tools_still_listed(
        self, _require_sap_creds: None, sap_mcp_client: "ClientSession"
    ) -> None:
