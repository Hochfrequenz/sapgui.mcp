...
class DesktopBackend:
    ...
    async def login(  # pylint: disable=unused-argument,too-many-arguments,too-many-positional-arguments
        ...
        try:
            # sapsucker.login.login() (>=0.5.0) closes any matching existing
            # connection up-front and verifies session.info.client / .user
            # against the requested values, raising SapConnectionError on
            # mismatch — see Hochfrequenz/sapsucker#26. The except block
            # below catches that and surfaces it as LoginResult.failure.
