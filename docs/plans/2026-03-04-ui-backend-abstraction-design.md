...
class SapUiPrimitives(Protocol):
    ...
    async def fill_field(self, label: str, value: str) -> FieldFillResult: ...
...
class SapUiInspection(Protocol):
    ...
    async def click_table_cell(self, row: int, col: int) -> ClickResult: ...
...
class SapPopup(Protocol):
    ...
    async def dismiss_popup(self, button_label: str | None = None) -> None: ...
...
These reduce the size of the final migration PR:
...
4. **Move `BrowserManager`** — relocate to `backend/webgui/browser.py`, re-export from old location

### Main migration PR (feature branch, atomic merge)

After preparatory PRs are on main:

1. Implement `WebGuiBackend` (wraps Playwright `Page`, calls JS helpers)
2. Implement `BackendManager` (session registry, singleton, `get_backend()`)
3. Refactor all `sap_*` and transaction tools to use `get_backend()` + protocol
4. Move `edit_helpers.py` logic into `WebGuiBackend` (editor protocol methods)
5. Make `browser_*` tool registration conditional on backend type
6. Remove old re-exports and direct Playwright imports from tools
7. Update/add tests
