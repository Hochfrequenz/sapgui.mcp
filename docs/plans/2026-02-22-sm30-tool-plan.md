# SM30 Table Maintenance Tool Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement a read-only SM30 Table Maintenance View lookup tool that dynamically parses flat table views from ARIA snapshots and returns structured row data.
**Architecture:** SM30 follows the established SE37/SE93 lookup pattern: Pydantic models (`sm30_models.py`), a dedicated parser (`sm30_parser.py`) for extracting dynamic column headers and row values from ARIA snapshots, and a tool module (`sm30_tools.py`) registered with FastMCP. The parser must handle variable column counts (3 to 20+) since SM30 views have no fixed schema.
**Tech Stack:** Python 3.12, Pydantic v2, FastMCP, Playwright (ARIA snapshots), pytest-anyio

---

### Task 1: Create Pydantic Models (`sm30_models.py`)

**Files:** Create `src/sapwebguimcp/models/sm30_models.py`

**Step 1:** Create the models file with `SM30Row`, `SM30ViewResult`, and `SM30FileSummary`:

```python
"""
Pydantic models for SM30 (Table Maintenance View) lookup tool.

These models represent view data retrieved from SM30 in display mode,
including dynamically-parsed column headers and row values.
"""

from typing import Any, Literal

from pydantic import AwareDatetime, BaseModel, Field

from sapwebguimcp.models.base import ToolResult

__all__ = [
    "SM30Row",
    "SM30ViewResult",
    "SM30FileSummary",
]

# View type detection
SM30ViewType = Literal["flat", "unsupported"]


class SM30Row(BaseModel):
    """A single row from an SM30 table maintenance view.

    Uses dict[str, str] since SM30 views contain configuration data
    that is best represented as strings (unlike SE16 which benefits
    from type coercion for numeric queries).
    """

    values: dict[str, str] = Field(description="Column name -> cell value")


class SM30ViewResult(ToolResult):
    """Result of SM30 table maintenance view lookup."""

    view_name: str = Field(description="View or table name that was looked up")
    description: str = Field(default="", description="View description from screen title")
    view_type: SM30ViewType = Field(description="Detected view type: 'flat' or 'unsupported'")
    columns: list[str] = Field(default_factory=list, description="Column names in display order")
    rows: list[SM30Row] = Field(default_factory=list, description="All rows from the view")
    row_count: int = Field(default=0, description="Number of rows returned")
    retrieved_at: AwareDatetime = Field(description="UTC timestamp when data was retrieved")


class SM30FileSummary(ToolResult):
    """Summary result when SM30 output is written to file.

    Returned instead of SM30ViewResult when output_file parameter is provided.
    Contains metadata and a preview of the first few rows.
    """

    output_file: str = Field(description="Path to JSON file containing full SM30ViewResult")
    view_name: str = Field(description="View or table name that was looked up")
    description: str = Field(default="", description="View description")
    view_type: SM30ViewType = Field(description="Detected view type")
    columns: list[str] = Field(default_factory=list, description="Column names in order")
    row_count: int = Field(default=0, description="Total rows written to file")
    sample_rows: list[SM30Row] = Field(default_factory=list, description="Preview of first 5 rows")
```

**Expected outcome:** File created, Pydantic models importable with `from sapwebguimcp.models.sm30_models import SM30Row, SM30ViewResult, SM30FileSummary`.

---

### Task 2: Register Models in `models/__init__.py`

**Files:** Modify `src/sapwebguimcp/models/__init__.py`

**Step 1:** Add import block for SM30 models. Insert after the SE93 models import block (around line 119):

```python
from sapwebguimcp.models.sm30_models import (
    SM30FileSummary,
    SM30Row,
    SM30ViewResult,
    SM30ViewType,
)
```

**Step 2:** Add SM30 models to the `__all__` list. Insert after the SE93 models section (around line 260):

```python
    # SM30 models
    "SM30FileSummary",
    "SM30Row",
    "SM30ViewResult",
    "SM30ViewType",
```

**Expected outcome:** `from sapwebguimcp.models import SM30Row, SM30ViewResult, SM30FileSummary, SM30ViewType` works.

**Verification:**
```bash
cd C:/github/sapwebgui.mcp
python -c "from sapwebguimcp.models import SM30Row, SM30ViewResult, SM30FileSummary, SM30ViewType; print('OK')"
```

---

### Task 3: Add SM30 Language Constants to `lang.py`

**Files:** Modify `src/sapwebguimcp/lang.py`

**Step 1:** Add SM30-specific constants after the SE93 section (before the Helper Functions section):

```python
# =============================================================================
# SM30 - Table Maintenance
# =============================================================================
SM30_DISPLAY_BUTTON_DE = "Anzeigen"
SM30_DISPLAY_BUTTON_EN = "Display"

SM30_MAINTAIN_BUTTON_DE = "Pflegen"
SM30_MAINTAIN_BUTTON_EN = "Maintain"

SM30_TABLE_VIEW_DE = "Tabelle/Sicht"
SM30_TABLE_VIEW_EN = "Table/View"

SM30_INITIAL_SCREEN_DE = "Tabellensicht-Pflege"
SM30_INITIAL_SCREEN_EN = "Table View Maintenance"

# SM30 status bar messages for view not found
SM30_NOT_FOUND_DE = "existiert nicht"
SM30_NOT_FOUND_EN = "does not exist"

# SM30 screen titles when displaying a view
SM30_DISPLAY_VIEW_DE = "Sicht"
SM30_DISPLAY_VIEW_EN = "View"

# SM30 hint for SM34 redirect
SM30_USE_SM34_DE = "SM34"
SM30_USE_SM34_EN = "SM34"

# SM30 column header for row selection (display mode)
SM30_ROW_SELECT_HINT_DE = "Zum Auswählen"
SM30_ROW_SELECT_HINT_EN = "To select"
```

**Expected outcome:** Constants available for use in parser.

---

### Task 4: Write Exploration Tests (Capture DE/EN Snapshots)

**Files:** Create `unittests/test_sm30_exploration.py`, create directory `unittests/testdata/sm30_exploration/`

**Step 1:** Create the exploration test file following the SE93 integration test pattern:

```python
"""
Exploratory tests for SM30 (Table Maintenance View) lookup tool.

These tests run against a real SAP system to capture YAML snapshots for
parser development. Run with SAP_LANGUAGE=DE and SAP_LANGUAGE=EN to get both.

Test views:
- V_T005 (Countries) - flat table, well-known, always has data
- V_T002 (Languages) - flat table, small
- ZZZNOTEXIST99 - non-existent view (error case)
- SM34 case - if a view requires SM34, detect and handle gracefully
"""

import os
from pathlib import Path

import pytest
from mcp import ClientSession

from sapwebguimcp.models import (
    FillFormResult,
    KeyboardResult,
    LoginResult,
    SnapshotResult,
    StatusBarInfo,
    TransactionResult,
)

from .conftest import call_tool_typed

YAML_SNAPSHOTS_DIR = Path(__file__).parent / "testdata" / "sm30_exploration"


async def capture_yaml_snapshot(
    client: ClientSession,
    base_name: str,
    overwrite: bool = False,
) -> str:
    """Capture YAML accessibility snapshot for parser development."""
    result = await call_tool_typed(client, "browser_snapshot", {}, SnapshotResult)
    yaml_content = result.snapshot

    language = os.environ.get("SAP_LANGUAGE", "de").lower()
    filename = f"{base_name}_{language}.yaml"
    filepath = YAML_SNAPSHOTS_DIR / filename

    if not filepath.exists() or overwrite:
        YAML_SNAPSHOTS_DIR.mkdir(parents=True, exist_ok=True)
        filepath.write_text(yaml_content, encoding="utf-8")
        print(f"Saved YAML snapshot: {filepath}")

    return yaml_content


async def _login_and_navigate_to_sm30(client: ClientSession) -> None:
    """Login and navigate to SM30."""
    login = await call_tool_typed(client, "sap_login", {}, LoginResult)
    assert login.success, f"Login failed: {login.error}"

    tx = await call_tool_typed(client, "sap_transaction", {"tcode": "SM30"}, TransactionResult)
    assert tx.success, f"Transaction failed: {tx.error}"

    await client.call_tool("browser_wait", {"timeout": 1000})


async def _fill_view_name_and_display(client: ClientSession, view_name: str) -> None:
    """Fill the view name and click Display button."""
    # Try German label first, then English
    fill = await call_tool_typed(
        client,
        "sap_fill_form",
        {"fields": {"Tabelle/Sicht": view_name}},
        FillFormResult,
    )
    if not fill.filled:
        fill = await call_tool_typed(
            client,
            "sap_fill_form",
            {"fields": {"Table/View": view_name}},
            FillFormResult,
        )
    assert fill.filled, f"Fill form failed for view '{view_name}': not_found={fill.not_found}"

    # Click Display button (Anzeigen/Display) - this is the radio button + button
    # Use F7 for display mode (consistent with SE37/SE93 pattern)
    keyboard = await call_tool_typed(client, "sap_keyboard", {"key": "F7"}, KeyboardResult)
    assert keyboard.success, f"F7 failed: {keyboard.error}"

    await client.call_tool("browser_wait", {"timeout": 2000})


# =============================================================================
# Exploration Tests
# =============================================================================


@pytest.mark.anyio
async def test_sm30_capture_initial_screen(sap_mcp_client: ClientSession) -> None:
    """Capture SM30 initial screen snapshot."""
    await _login_and_navigate_to_sm30(sap_mcp_client)
    await capture_yaml_snapshot(sap_mcp_client, "sm30_initial", overwrite=True)
    print("SM30 initial screen snapshot saved")


@pytest.mark.anyio
async def test_sm30_capture_v_t005_countries(sap_mcp_client: ClientSession) -> None:
    """Capture SM30 display of V_T005 (Countries) - flat table view."""
    await _login_and_navigate_to_sm30(sap_mcp_client)
    await _fill_view_name_and_display(sap_mcp_client, "V_T005")
    await capture_yaml_snapshot(sap_mcp_client, "sm30_v_t005_display", overwrite=True)
    print("V_T005 (Countries) display snapshot saved")


@pytest.mark.anyio
async def test_sm30_capture_v_t002_languages(sap_mcp_client: ClientSession) -> None:
    """Capture SM30 display of V_T002 (Languages) - flat table view."""
    await _login_and_navigate_to_sm30(sap_mcp_client)
    await _fill_view_name_and_display(sap_mcp_client, "V_T002")
    await capture_yaml_snapshot(sap_mcp_client, "sm30_v_t002_display", overwrite=True)
    print("V_T002 (Languages) display snapshot saved")


@pytest.mark.anyio
async def test_sm30_capture_not_found(sap_mcp_client: ClientSession) -> None:
    """Capture SM30 behavior for non-existent view."""
    await _login_and_navigate_to_sm30(sap_mcp_client)
    await _fill_view_name_and_display(sap_mcp_client, "ZZZNOTEXIST99")

    status = await call_tool_typed(sap_mcp_client, "sap_read_status_bar", {}, StatusBarInfo)
    print(f"Status bar for non-existent view: {status.message}")

    await capture_yaml_snapshot(sap_mcp_client, "sm30_not_found", overwrite=True)
    print("Not found snapshot saved")


@pytest.mark.anyio
async def test_sm30_capture_sm34_view(sap_mcp_client: ClientSession) -> None:
    """
    Capture SM30 behavior for a view that requires SM34 (extended maintenance).

    V_T005X or similar cluster views may redirect to SM34.
    If no SM34 redirect occurs, the snapshot still shows what happens.
    """
    await _login_and_navigate_to_sm30(sap_mcp_client)

    # Try a view that might require SM34 - TVDIR is maintenance view directory
    await _fill_view_name_and_display(sap_mcp_client, "V_TVDIR")

    status = await call_tool_typed(sap_mcp_client, "sap_read_status_bar", {}, StatusBarInfo)
    print(f"Status bar for SM34 view: {status.message}")

    await capture_yaml_snapshot(sap_mcp_client, "sm30_sm34_redirect", overwrite=True)
    print("SM34 redirect snapshot saved")
```

**Expected outcome:** Running these tests against a real SAP system creates snapshot files in `unittests/testdata/sm30_exploration/`:
- `sm30_initial_de.yaml` / `sm30_initial_en.yaml`
- `sm30_v_t005_display_de.yaml` / `sm30_v_t005_display_en.yaml`
- `sm30_v_t002_display_de.yaml` / `sm30_v_t002_display_en.yaml`
- `sm30_not_found_de.yaml` / `sm30_not_found_en.yaml`
- `sm30_sm34_redirect_de.yaml` / `sm30_sm34_redirect_en.yaml`

**Run command:**
```bash
cd C:/github/sapwebgui.mcp
SAP_LANGUAGE=DE pytest unittests/test_sm30_exploration.py -v -s --no-header 2>&1 | head -100
SAP_LANGUAGE=EN pytest unittests/test_sm30_exploration.py -v -s --no-header 2>&1 | head -100
```

---

### Task 5: Write SM30 Parser (`sm30_parser.py`)

**Files:** Create `src/sapwebguimcp/parsers/sm30_parser.py`

**Step 1:** Create the parser. SM30 display mode renders a standard ALV-like grid. The parser must:
1. Detect if the view loaded successfully (vs. not found / SM34 redirect)
2. Extract column headers dynamically from `columnheader` elements
3. Extract rows from `gridcell` elements
4. Handle variable column counts (3 to 20+)

```python
"""
Parser for SM30 (Table Maintenance View) ARIA snapshots.

Extracts structured row data from SM30 display mode grids, handling:
- Dynamic column headers (views have 3 to 20+ columns)
- Data rows with gridcell values
- View type detection (flat vs unsupported/SM34)
- German and English label support
- View not found detection
"""

import logging
import re
from datetime import UTC, datetime

from sapwebguimcp.lang import (
    SM30_NOT_FOUND_DE,
    SM30_NOT_FOUND_EN,
    SM30_ROW_SELECT_HINT_DE,
    SM30_ROW_SELECT_HINT_EN,
    SM30_USE_SM34_DE,
    SM30_USE_SM34_EN,
)
from sapwebguimcp.models.sm30_models import SM30Row, SM30ViewResult, SM30ViewType

logger = logging.getLogger(__name__)

__all__ = [
    "parse_sm30_snapshot",
    "parse_sm30_columns",
    "parse_sm30_rows",
    "detect_sm30_view_type",
]

# =============================================================================
# Regex Patterns
# =============================================================================

# Column header extraction from grid
_COLUMNHEADER_PATTERN = re.compile(r'columnheader "(?P<name>[^"]+)"')

# Header row pattern (row containing columnheader elements)
_HEADER_ROW_PATTERN = re.compile(
    r'- row "[^"]*":\s*\n(?:\s*- columnheader "[^"]+"[^\n]*\n?)+',
    re.MULTILINE,
)

# Data row pattern - SM30 display mode rows
# SM30 data rows start with "To select..." or "Zum Auswählen..." hint text
_ROW_START_PATTERN = re.compile(
    rf'- \'?row "(?:{re.escape(SM30_ROW_SELECT_HINT_DE)}|{re.escape(SM30_ROW_SELECT_HINT_EN)})'
)

# Gridcell value extraction
_GRIDCELL_WITH_VALUE_PATTERN = re.compile(r'gridcell "(?P<value>[^"]*)"')
_GRIDCELL_EMPTY_PATTERN = re.compile(r"^\s*- gridcell:\s*$")

# Row selection column names to skip
_SELECTION_COLUMN_NAMES = {
    "Spalte für Zeilenauswahl",
    "Column for row selection",
}

# SM34 detection patterns
_SM34_PATTERN = re.compile(
    rf"(?:{re.escape(SM30_USE_SM34_DE)}|{re.escape(SM30_USE_SM34_EN)})",
    re.IGNORECASE,
)

# View not found patterns
_NOT_FOUND_PATTERNS = [
    SM30_NOT_FOUND_DE,
    SM30_NOT_FOUND_EN,
    "does not exist",
    "existiert nicht",
    "nicht vorhanden",
    "not found",
]

# View description from page title
# DE: 'heading "Sicht "Länder" anzeigen: Übersicht"'
# EN: 'heading "Display View "Countries": Overview"'
_VIEW_TITLE_PATTERN = re.compile(
    r'heading "(?:Sicht|Display View)\s+"(?P<desc>[^"]+)"\s+(?:anzeigen|display)',
    re.IGNORECASE,
)

# Alternative: extract from generic heading
_HEADING_PATTERN = re.compile(r'heading "(?P<title>[^"]+)"')


# =============================================================================
# Detection Functions
# =============================================================================


def detect_sm30_view_type(snapshot: str) -> SM30ViewType:
    """
    Detect whether the SM30 snapshot shows a flat table or an unsupported view type.

    Returns "flat" if a grid with columnheaders is found.
    Returns "unsupported" if SM34 redirect is detected or no grid found.
    """
    # Check for SM34 redirect message
    if _SM34_PATTERN.search(snapshot):
        return "unsupported"

    # Check for grid with column headers (indicates flat table)
    if _COLUMNHEADER_PATTERN.search(snapshot):
        return "flat"

    return "unsupported"


def _is_view_not_found(snapshot: str) -> bool:
    """Check if the snapshot indicates the view was not found."""
    snapshot_lower = snapshot.lower()
    return any(pattern.lower() in snapshot_lower for pattern in _NOT_FOUND_PATTERNS)


def _extract_view_description(snapshot: str) -> str:
    """Extract the view description from the page heading."""
    match = _VIEW_TITLE_PATTERN.search(snapshot)
    if match:
        return match.group("desc")

    # Fallback: use first heading
    match = _HEADING_PATTERN.search(snapshot)
    if match:
        return match.group("title")

    return ""


# =============================================================================
# Column Parsing
# =============================================================================


def parse_sm30_columns(snapshot: str) -> list[str]:
    """
    Extract column names from the SM30 grid header row.

    Skips the row selection column (present in display mode).
    Handles variable column counts dynamically.

    Args:
        snapshot: ARIA snapshot YAML content

    Returns:
        List of column names in order
    """
    columns: list[str] = []

    # Find the header row section
    header_match = _HEADER_ROW_PATTERN.search(snapshot)
    if header_match:
        header_section = header_match.group(0)
    else:
        # Fallback: search from grid start to first data row
        grid_start = snapshot.find("- grid:")
        if grid_start == -1:
            grid_start = snapshot.find("grid:")
        if grid_start == -1:
            logger.warning("No grid found in SM30 snapshot")
            return columns

        # Find first data row
        first_row_de = snapshot.find(f'row "{SM30_ROW_SELECT_HINT_DE}', grid_start)
        first_row_en = snapshot.find(f'row "{SM30_ROW_SELECT_HINT_EN}', grid_start)

        candidates = [pos for pos in [first_row_de, first_row_en] if pos != -1]
        first_row = min(candidates) if candidates else len(snapshot)

        header_section = snapshot[grid_start:first_row]

    # Extract all columnheader values
    for match in _COLUMNHEADER_PATTERN.finditer(header_section):
        col_name = match.group("name")
        # Skip the row selection column
        if col_name not in _SELECTION_COLUMN_NAMES:
            columns.append(col_name)

    return columns


# =============================================================================
# Row Parsing
# =============================================================================


def _extract_gridcell_value(line: str) -> str | None:
    """Extract value from a gridcell line, returning None if not a gridcell."""
    match = _GRIDCELL_WITH_VALUE_PATTERN.search(line)
    if match:
        return match.group("value")
    if _GRIDCELL_EMPTY_PATTERN.match(line):
        return ""
    return None


def parse_sm30_rows(snapshot: str, columns: list[str] | None = None) -> list[SM30Row]:
    """
    Extract data rows from SM30 display grid snapshot.

    Each row is returned as an SM30Row with values dict mapping column name to value.

    Args:
        snapshot: ARIA snapshot YAML content
        columns: Column names (if None, will be extracted from snapshot)

    Returns:
        List of SM30Row instances
    """
    if columns is None:
        columns = parse_sm30_columns(snapshot)

    if not columns:
        logger.warning("No columns found, cannot parse SM30 rows")
        return []

    rows: list[SM30Row] = []
    lines = snapshot.split("\n")
    in_data_row = False
    current_cells: list[str] = []

    for line in lines:
        # Check for start of a data row
        if _ROW_START_PATTERN.search(line):
            # Save previous row if valid
            _save_row_if_valid(current_cells, columns, rows)
            current_cells = []
            in_data_row = True
            continue

        if in_data_row:
            # Check for end of data section
            if "- rowgroup" in line or "- menu:" in line:
                _save_row_if_valid(current_cells, columns, rows)
                in_data_row = False
                current_cells = []
                continue

            # Extract gridcell value
            cell_value = _extract_gridcell_value(line)
            if cell_value is not None:
                current_cells.append(cell_value)

    # Don't forget the last row
    if in_data_row:
        _save_row_if_valid(current_cells, columns, rows)

    return rows


def _save_row_if_valid(
    current_cells: list[str],
    columns: list[str],
    rows: list[SM30Row],
) -> None:
    """Save current cells as an SM30Row if valid (has data beyond selection cell)."""
    if current_cells and len(current_cells) > 1:
        # Skip first cell (row selection column)
        data_cells = current_cells[1:]
        values: dict[str, str] = {}
        for i, col_name in enumerate(columns):
            if i < len(data_cells):
                values[col_name] = data_cells[i]
            else:
                values[col_name] = ""
        if values:
            rows.append(SM30Row(values=values))


# =============================================================================
# Main Parser
# =============================================================================


def parse_sm30_snapshot(
    snapshot: str,
    view_name: str,
) -> SM30ViewResult:
    """
    Parse SM30 display snapshot into structured data.

    This is the main entry point. Handles:
    - View not found
    - SM34 redirect detection
    - Flat table parsing (dynamic columns + rows)

    Args:
        snapshot: The YAML accessibility snapshot from browser_snapshot
        view_name: The view/table name being looked up

    Returns:
        SM30ViewResult with parsed data or error
    """
    now = datetime.now(UTC)

    # Check for view not found
    if _is_view_not_found(snapshot):
        return SM30ViewResult.failure(
            error=f"View '{view_name}' not found",
            view_name=view_name,
            description="",
            view_type="unsupported",
            columns=[],
            rows=[],
            row_count=0,
            retrieved_at=now,
        )

    # Detect view type
    view_type = detect_sm30_view_type(snapshot)

    if view_type == "unsupported":
        return SM30ViewResult.failure(
            error=(
                f"View '{view_name}' requires SM34 (Extended Table Maintenance) or has "
                "an unsupported layout. Only flat table views are supported."
            ),
            view_name=view_name,
            description=_extract_view_description(snapshot),
            view_type="unsupported",
            columns=[],
            rows=[],
            row_count=0,
            retrieved_at=now,
        )

    # Parse flat table
    description = _extract_view_description(snapshot)
    columns = parse_sm30_columns(snapshot)
    rows = parse_sm30_rows(snapshot, columns)

    return SM30ViewResult(
        view_name=view_name,
        description=description,
        view_type="flat",
        columns=columns,
        rows=rows,
        row_count=len(rows),
        retrieved_at=now,
    )
```

**Expected outcome:** Parser module created. Functions `parse_sm30_snapshot`, `parse_sm30_columns`, `parse_sm30_rows`, `detect_sm30_view_type` are importable.

**Verification:**
```bash
python -c "from sapwebguimcp.parsers.sm30_parser import parse_sm30_snapshot; print('OK')"
```

---

### Task 6: Write Parser Unit Tests

**Files:** Create `unittests/test_sm30_parser.py`

**Step 1:** Create parser unit tests that work against captured snapshots AND synthetic inline snapshots:

```python
"""Unit tests for SM30 (Table Maintenance View) parser."""

from pathlib import Path

import pytest

from sapwebguimcp.parsers.sm30_parser import (
    detect_sm30_view_type,
    parse_sm30_columns,
    parse_sm30_rows,
    parse_sm30_snapshot,
)

# Test data directory
TESTDATA_DIR = Path(__file__).parent / "testdata" / "sm30_exploration"


class TestDetectViewType:
    """Tests for detect_sm30_view_type function."""

    def test_flat_table_detected(self) -> None:
        """A snapshot with columnheader elements should be detected as flat."""
        snapshot = """
        - grid:
          - row "Header":
            - columnheader "Country"
            - columnheader "Name"
        """
        assert detect_sm30_view_type(snapshot) == "flat"

    def test_sm34_redirect_detected(self) -> None:
        """A snapshot mentioning SM34 should be detected as unsupported."""
        snapshot = 'status "Use transaction SM34 for this view"'
        assert detect_sm30_view_type(snapshot) == "unsupported"

    def test_no_grid_is_unsupported(self) -> None:
        """A snapshot without grid or columnheaders should be unsupported."""
        snapshot = 'heading "Some random screen"'
        assert detect_sm30_view_type(snapshot) == "unsupported"

    def test_real_snapshot_de(self) -> None:
        """Test detection from real DE snapshot if available."""
        snapshot_file = TESTDATA_DIR / "sm30_v_t005_display_de.yaml"
        if snapshot_file.exists():
            snapshot = snapshot_file.read_text(encoding="utf-8")
            assert detect_sm30_view_type(snapshot) == "flat"

    def test_real_snapshot_en(self) -> None:
        """Test detection from real EN snapshot if available."""
        snapshot_file = TESTDATA_DIR / "sm30_v_t005_display_en.yaml"
        if snapshot_file.exists():
            snapshot = snapshot_file.read_text(encoding="utf-8")
            assert detect_sm30_view_type(snapshot) == "flat"


class TestParseColumns:
    """Tests for parse_sm30_columns function."""

    def test_parse_simple_columns(self) -> None:
        """Test extracting column names from a simple grid."""
        snapshot = """
      - row "Column for row selection Country Name":
        - columnheader "Column for row selection": To select all
        - columnheader "Country"
        - columnheader "Name"
        """
        columns = parse_sm30_columns(snapshot)
        assert columns == ["Country", "Name"]

    def test_skips_selection_column(self) -> None:
        """Selection column should be skipped."""
        snapshot = """
      - row "header row":
        - columnheader "Column for row selection"
        - columnheader "Field1"
        - columnheader "Field2"
        """
        columns = parse_sm30_columns(snapshot)
        assert "Column for row selection" not in columns
        assert columns == ["Field1", "Field2"]

    def test_skips_selection_column_de(self) -> None:
        """German selection column should be skipped."""
        snapshot = """
      - row "header row":
        - columnheader "Spalte für Zeilenauswahl"
        - columnheader "Land"
        - columnheader "Bezeichnung"
        """
        columns = parse_sm30_columns(snapshot)
        assert "Spalte für Zeilenauswahl" not in columns
        assert columns == ["Land", "Bezeichnung"]

    def test_many_columns(self) -> None:
        """Test parsing many columns (SM30 views can have 20+)."""
        headers = "\n".join(
            f'        - columnheader "Col{i}"' for i in range(20)
        )
        snapshot = f"""
      - row "header":
        - columnheader "Column for row selection"
{headers}
        """
        columns = parse_sm30_columns(snapshot)
        assert len(columns) == 20

    def test_real_v_t005_snapshot(self) -> None:
        """Test parsing columns from real V_T005 snapshot if available."""
        for lang in ["de", "en"]:
            snapshot_file = TESTDATA_DIR / f"sm30_v_t005_display_{lang}.yaml"
            if snapshot_file.exists():
                snapshot = snapshot_file.read_text(encoding="utf-8")
                columns = parse_sm30_columns(snapshot)
                assert len(columns) > 0, f"No columns found in {lang} snapshot"


class TestParseRows:
    """Tests for parse_sm30_rows function."""

    def test_parse_simple_rows(self) -> None:
        """Test parsing rows from a simple grid."""
        snapshot = """
      - row "Column for row selection Country Name":
        - columnheader "Column for row selection"
        - columnheader "Country"
        - columnheader "Name"
      - row "To select a row ABC Germany":
        - gridcell "To select a row"
        - gridcell "DE":
          - textbox
        - gridcell "Germany":
          - textbox
      - row "To select a row XYZ France":
        - gridcell "To select a row"
        - gridcell "FR":
          - textbox
        - gridcell "France":
          - textbox
    - rowgroup
        """
        rows = parse_sm30_rows(snapshot)
        assert len(rows) == 2
        assert rows[0].values["Country"] == "DE"
        assert rows[0].values["Name"] == "Germany"
        assert rows[1].values["Country"] == "FR"
        assert rows[1].values["Name"] == "France"

    def test_parse_empty_cells(self) -> None:
        """Test handling of empty cells."""
        snapshot = """
      - row "header":
        - columnheader "Column for row selection"
        - columnheader "A"
        - columnheader "B"
      - row "To select a row data":
        - gridcell "To select a row"
        - gridcell "Val1":
          - textbox
        - gridcell:
          - textbox
    - rowgroup
        """
        rows = parse_sm30_rows(snapshot)
        assert len(rows) == 1
        assert rows[0].values["A"] == "Val1"
        assert rows[0].values["B"] == ""

    def test_parse_german_rows(self) -> None:
        """Test parsing rows with German hint text."""
        snapshot = """
      - row "header":
        - columnheader "Spalte für Zeilenauswahl"
        - columnheader "Land"
        - columnheader "Bezeichnung"
      - row "Zum Auswählen DE Deutschland":
        - gridcell "Zum Auswählen"
        - gridcell "DE":
          - textbox
        - gridcell "Deutschland":
          - textbox
    - rowgroup
        """
        rows = parse_sm30_rows(snapshot)
        assert len(rows) == 1
        assert rows[0].values["Land"] == "DE"
        assert rows[0].values["Bezeichnung"] == "Deutschland"

    def test_real_v_t005_snapshot(self) -> None:
        """Test parsing rows from real V_T005 snapshot if available."""
        for lang in ["de", "en"]:
            snapshot_file = TESTDATA_DIR / f"sm30_v_t005_display_{lang}.yaml"
            if snapshot_file.exists():
                snapshot = snapshot_file.read_text(encoding="utf-8")
                rows = parse_sm30_rows(snapshot)
                assert len(rows) > 0, f"No rows found in {lang} snapshot"


class TestParseSm30Snapshot:
    """Tests for the main parse_sm30_snapshot function."""

    def test_view_not_found(self) -> None:
        """Test that not-found views return error result."""
        snapshot = 'status "View ZZZNOTEXIST99 does not exist"'
        result = parse_sm30_snapshot(snapshot, "ZZZNOTEXIST99")
        assert not result.success
        assert "not found" in result.error.lower()
        assert result.view_type == "unsupported"

    def test_view_not_found_de(self) -> None:
        """Test German not-found message."""
        snapshot = 'status "Sicht ZZZNOTEXIST99 existiert nicht"'
        result = parse_sm30_snapshot(snapshot, "ZZZNOTEXIST99")
        assert not result.success

    def test_sm34_redirect(self) -> None:
        """Test SM34 redirect detection."""
        snapshot = 'status "Please use SM34 for this maintenance view"'
        result = parse_sm30_snapshot(snapshot, "V_CLUSTER")
        assert not result.success
        assert "SM34" in result.error
        assert result.view_type == "unsupported"

    def test_successful_flat_table(self) -> None:
        """Test parsing a successful flat table view."""
        snapshot = """
- heading "Display View \\"Countries\\": Overview"
  - grid:
    - row "header":
      - columnheader "Column for row selection"
      - columnheader "Country"
      - columnheader "Name"
    - row "To select a row DE Germany":
      - gridcell "To select a row"
      - gridcell "DE":
        - textbox
      - gridcell "Germany":
        - textbox
  - rowgroup
        """
        result = parse_sm30_snapshot(snapshot, "V_T005")
        assert result.success
        assert result.view_type == "flat"
        assert result.view_name == "V_T005"
        assert result.columns == ["Country", "Name"]
        assert result.row_count == 1
        assert result.rows[0].values["Country"] == "DE"

    def test_real_v_t005_de(self) -> None:
        """Test full parse from real DE snapshot if available."""
        snapshot_file = TESTDATA_DIR / "sm30_v_t005_display_de.yaml"
        if snapshot_file.exists():
            snapshot = snapshot_file.read_text(encoding="utf-8")
            result = parse_sm30_snapshot(snapshot, "V_T005")
            assert result.success
            assert result.view_type == "flat"
            assert len(result.columns) > 0
            assert result.row_count > 0

    def test_real_v_t005_en(self) -> None:
        """Test full parse from real EN snapshot if available."""
        snapshot_file = TESTDATA_DIR / "sm30_v_t005_display_en.yaml"
        if snapshot_file.exists():
            snapshot = snapshot_file.read_text(encoding="utf-8")
            result = parse_sm30_snapshot(snapshot, "V_T005")
            assert result.success
            assert result.view_type == "flat"
            assert len(result.columns) > 0
            assert result.row_count > 0

    def test_real_not_found_de(self) -> None:
        """Test not-found from real DE snapshot if available."""
        snapshot_file = TESTDATA_DIR / "sm30_not_found_de.yaml"
        if snapshot_file.exists():
            snapshot = snapshot_file.read_text(encoding="utf-8")
            result = parse_sm30_snapshot(snapshot, "ZZZNOTEXIST99")
            assert not result.success


if __name__ == "__main__":
    pytest.main([__file__, "-v"])
```

**Expected outcome:** All synthetic tests pass immediately. Real snapshot tests pass once exploration tests have been run.

**Verification:**
```bash
cd C:/github/sapwebgui.mcp
pytest unittests/test_sm30_parser.py -v --no-header 2>&1 | tail -30
```

---

### Task 7: Write the SM30 Tool (`sm30_tools.py`)

**Files:** Create `src/sapwebguimcp/tools/sm30_tools.py`

**Step 1:** Create the tool module following the SE37 lookup pattern:

```python
"""
SM30 (Table Maintenance View) read-only lookup tool.

This module provides a tool to display SM30 table maintenance views,
returning structured data with dynamically-parsed columns and rows.
"""

import json
import logging
import re
from datetime import UTC, datetime
from pathlib import Path
from typing import Any

from fastmcp import FastMCP
from mcp.types import ToolAnnotations

from sapwebguimcp.models import get_browser_manager
from sapwebguimcp.models.sm30_models import SM30FileSummary, SM30Row, SM30ViewResult
from sapwebguimcp.parsers.sm30_parser import parse_sm30_snapshot
from sapwebguimcp.tools.sap_tool_impl import sap_transaction_impl

logger = logging.getLogger(__name__)

__all__ = ["register_sm30_tools"]


# =============================================================================
# SM30 Navigation Helpers
# =============================================================================


async def _find_view_field(page: Any) -> Any:
    """Find the Table/View input field in SM30 using multiple strategies."""
    strategies = [
        # Strategy 1: Try by role with exact name matches (DE/EN)
        page.get_by_role("textbox", name="Tabelle/Sicht"),
        page.get_by_role("textbox", name="Table/View"),
        # Strategy 2: Try regex pattern
        page.get_by_role(
            "textbox",
            name=re.compile(r"Tabelle/Sicht|Table/View", re.I),
        ),
        # Strategy 3: Try by title attribute
        page.locator(
            "input[title*='Tabelle'], input[title*='Table/View']"
        ).first,
        # Strategy 4: First visible input field (last resort)
        page.locator("input:visible").first,
    ]

    for field in strategies:
        if await field.count() > 0:
            return field

    return None


async def _fill_view_field(page: Any, view_name: str) -> str | None:
    """Fill the view name field in SM30. Returns error string or None."""
    view_field = await _find_view_field(page)

    if view_field is None or await view_field.count() == 0:
        return "Could not find Table/View field in SM30"

    # Clear the field
    await view_field.click(click_count=3)
    await page.wait_for_timeout(100)
    await page.keyboard.press("Delete")
    await page.wait_for_timeout(50)

    # Type the view name
    await page.keyboard.type(view_name.upper())
    await page.wait_for_timeout(100)
    return None


async def _ensure_display_mode(page: Any) -> None:
    """
    Ensure the Display radio button is selected (not Maintain).

    SM30 initial screen has two radio buttons:
    - DE: "Anzeigen" / EN: "Display"
    - DE: "Pflegen" / EN: "Maintain"

    We must select Display for read-only access.
    """
    # Try to find and click the Display radio button
    for label in ["Anzeigen", "Display"]:
        radio = page.get_by_role("radio", name=label)
        if await radio.count() > 0:
            await radio.click()
            await page.wait_for_timeout(200)
            return

    # If radio buttons not found, just proceed (F7 forces display mode anyway)
    logger.debug("Display radio button not found, proceeding with default")


async def _check_view_not_found(page: Any, view_name: str) -> str | None:
    """
    Check if the view was not found after pressing Enter/F7.

    Returns error message if not found, None if view loaded successfully.
    """
    # Check status bar for error messages
    status_bar = page.locator("#sapStatusBarAll, [id*='STATUSBAR']").first
    status_text = await status_bar.text_content() if await status_bar.count() > 0 else ""

    not_found_msgs = {
        "existiert nicht",
        "does not exist",
        "nicht vorhanden",
        "not found",
        "ist nicht vorhanden",
        "kein Pflegedialog",
        "no maintenance dialog",
    }

    if status_text and any(msg in status_text.lower() for msg in not_found_msgs):
        return f"View '{view_name}' not found: {status_text.strip()}"

    # Check page title - if still on initial screen, view wasn't found
    page_title = await page.title()
    is_initial = (
        "Tabellensicht-Pflege" in page_title
        or "Table View Maintenance" in page_title
        or "Einstieg" in page_title
        or "Initial" in page_title
    )

    if is_initial:
        # Still on initial screen
        if status_text:
            return f"View '{view_name}' not found: {status_text.strip()}"
        return f"View '{view_name}' not found (still on initial screen)"

    return None


async def _check_sm34_redirect(page: Any) -> str | None:
    """
    Check if SM30 redirected to SM34 (extended table maintenance).

    Returns error message if SM34 redirect detected, None otherwise.
    """
    # Check status bar for SM34 mention
    status_bar = page.locator("#sapStatusBarAll, [id*='STATUSBAR']").first
    status_text = await status_bar.text_content() if await status_bar.count() > 0 else ""

    if status_text and "sm34" in status_text.lower():
        return f"This view requires SM34 (Extended Table Maintenance): {status_text.strip()}"

    # Check snapshot for SM34 indicators
    snapshot = await page.locator("body").aria_snapshot()
    if "SM34" in snapshot:
        return "This view requires SM34 (Extended Table Maintenance)"

    return None


async def _collect_all_rows_with_pagination(
    page: Any,
    initial_snapshot: str,
) -> str:
    """
    Collect rows from SM30 by scrolling through all pages.

    SM30 display mode uses ALV grid with lazy loading, similar to SE16.
    We scroll with PageDown and collect snapshots until no new rows appear.

    For SM30, we collect the initial snapshot. If the view has more rows
    than visible, we paginate. However, most SM30 views have < 100 rows
    so single-page is common.

    Args:
        page: Playwright page object
        initial_snapshot: The first page snapshot

    Returns:
        Combined snapshot string (we re-parse at the end)
    """
    # For now, return initial snapshot. Pagination can be added if needed
    # since most SM30 views have fewer rows than the visible area.
    # TODO: Add pagination for large views (similar to SE16 pattern)
    return initial_snapshot


async def _lookup_view(page: Any, view_name: str) -> SM30ViewResult:
    """Look up a single SM30 view."""
    now = datetime.now(UTC)

    # Navigate to SM30
    tx_result = await sap_transaction_impl("SM30")
    if not tx_result.success:
        return SM30ViewResult.failure(
            error=f"Failed to navigate to SM30: {tx_result.error}",
            view_name=view_name,
            description="",
            view_type="unsupported",
            columns=[],
            rows=[],
            row_count=0,
            retrieved_at=now,
        )

    # Wait for SM30 screen to be ready
    await page.wait_for_timeout(500)
    await page.wait_for_load_state("networkidle")

    # Ensure display mode is selected
    await _ensure_display_mode(page)

    # Fill view name
    fill_error = await _fill_view_field(page, view_name)
    if fill_error:
        return SM30ViewResult.failure(
            error=fill_error,
            view_name=view_name,
            description="",
            view_type="unsupported",
            columns=[],
            rows=[],
            row_count=0,
            retrieved_at=now,
        )

    # Press Enter to display the view (Display button shortcut)
    await page.wait_for_timeout(300)
    await page.keyboard.press("Enter")
    await page.wait_for_timeout(1000)
    await page.wait_for_load_state("networkidle")
    await page.wait_for_timeout(500)

    # Check for view not found
    not_found_error = await _check_view_not_found(page, view_name)
    if not_found_error:
        return SM30ViewResult.failure(
            error=not_found_error,
            view_name=view_name,
            description="",
            view_type="unsupported",
            columns=[],
            rows=[],
            row_count=0,
            retrieved_at=now,
        )

    # Check for SM34 redirect
    sm34_error = await _check_sm34_redirect(page)
    if sm34_error:
        return SM30ViewResult.failure(
            error=sm34_error,
            view_name=view_name,
            description="",
            view_type="unsupported",
            columns=[],
            rows=[],
            row_count=0,
            retrieved_at=now,
        )

    # Get snapshot and parse
    snapshot = await page.locator("body").aria_snapshot()
    logger.debug(
        "Got SM30 snapshot",
        extra={"view": view_name, "length": len(snapshot)},
    )

    return parse_sm30_snapshot(snapshot, view_name)


# =============================================================================
# MCP Tool Registration
# =============================================================================


def register_sm30_tools(mcp: FastMCP) -> None:
    """Register SM30 tools with the MCP server."""

    @mcp.tool(
        annotations=ToolAnnotations(
            readOnlyHint=True,
            openWorldHint=False,
        ),
        description=(
            "Look up SAP table maintenance view entries from SM30 (read-only display mode). "
            "USE THIS instead of sap_transaction('SM30') - faster and returns structured data. "
            "Returns all entries from the maintenance view with dynamically-parsed columns. "
            "Supports any flat table view (e.g., V_T005, V_T002, custom Z* views). "
            "Non-flat views (SM34/cluster/hierarchical) are detected and reported as unsupported."
        ),
    )
    async def sap_sm30_lookup(
        view_name: str,
        output_file: str | None = None,
        session: str | None = None,
        agent_id: str | None = None,
    ) -> SM30ViewResult | SM30FileSummary:
        """
        Look up table maintenance view entries from SM30.

        Args:
            view_name: The maintenance view or table name (e.g., 'V_T005', 'V_T002')
            output_file: If provided, write full results to this JSON file and return summary.
            session: Session ID (e.g., "s1", "s2"). None uses primary session.
            agent_id: Agent identifier for binding check. Optional.

        Returns:
            SM30ViewResult with view data (inline), or
            SM30FileSummary with file path and preview (when output_file provided)
        """
        now = datetime.now(UTC)
        browser_manager = await get_browser_manager()

        try:
            page = browser_manager.get_session_page_checked(
                session, agent_id, "sap_sm30_lookup"
            )
        except ValueError as e:
            return SM30ViewResult.failure(
                error=f"Session error: {e}",
                view_name=view_name,
                description="",
                view_type="unsupported",
                columns=[],
                rows=[],
                row_count=0,
                retrieved_at=now,
            )

        try:
            result = await _lookup_view(page, view_name)
        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.exception("Looking up SM30 view", extra={"view": view_name})
            result = SM30ViewResult.failure(
                error=f"Error looking up view '{view_name}': {e}",
                view_name=view_name,
                description="",
                view_type="unsupported",
                columns=[],
                rows=[],
                row_count=0,
                retrieved_at=now,
            )

        # Write to file if requested
        if output_file and result.success:
            output_path = Path(output_file)
            output_path.parent.mkdir(parents=True, exist_ok=True)

            with output_path.open("w", encoding="utf-8") as f:
                json.dump(
                    result.model_dump(mode="json"), f, indent=2, ensure_ascii=False
                )

            return SM30FileSummary(
                success=True,
                output_file=str(output_path.absolute()),
                view_name=result.view_name,
                description=result.description,
                view_type=result.view_type,
                columns=result.columns,
                row_count=result.row_count,
                sample_rows=result.rows[:5],
            )

        return result
```

**Expected outcome:** Tool module created with `register_sm30_tools` function that registers `sap_sm30_lookup` tool.

---

### Task 8: Register Tool in `tools/__init__.py` and `server.py`

**Files:** Modify `src/sapwebguimcp/tools/__init__.py`, Modify `src/sapwebguimcp/server.py`

**Step 1:** Add import in `tools/__init__.py`. Insert after the SE93 tools import (around line 34):

```python
from sapwebguimcp.tools.sm30_tools import register_sm30_tools
```

**Step 2:** Add to `__all__` in `tools/__init__.py`. Insert alphabetically:

```python
    "register_sm30_tools",
```

**Step 3:** In `server.py`, add import. Insert after the `register_se93_tools` import (around line 39):

```python
    register_sm30_tools,
```

**Step 4:** In `server.py`, add registration call. Insert after `register_se93_tools(mcp)` (around line 169):

```python
register_sm30_tools(mcp)
```

**Expected outcome:** The SM30 tool is registered and appears in the tool list.

**Verification:**
```bash
python -c "
from sapwebguimcp.tools import register_sm30_tools
print('Import OK')
"
```

---

### Task 9: Write Integration Test

**Files:** Create `unittests/test_sm30_integration.py`

**Step 1:** Create integration test following the SE93 integration test pattern:

```python
"""
Integration tests for SM30 (Table Maintenance View) lookup tool.

These tests run against a real SAP system to verify the sap_sm30_lookup tool
works correctly end-to-end.
"""

import pytest
from mcp import ClientSession

from sapwebguimcp.models import LoginResult
from sapwebguimcp.models.sm30_models import SM30ViewResult

from .conftest import call_tool_typed


# =============================================================================
# Integration Tests
# =============================================================================


@pytest.mark.anyio
async def test_sm30_lookup_v_t005_countries(sap_mcp_client: ClientSession) -> None:
    """
    Test sap_sm30_lookup with V_T005 (Countries) - a well-known flat table view.
    """
    login = await call_tool_typed(sap_mcp_client, "sap_login", {}, LoginResult)
    assert login.success, f"Login failed: {login.error}"

    result = await call_tool_typed(
        sap_mcp_client,
        "sap_sm30_lookup",
        {"view_name": "V_T005"},
        SM30ViewResult,
    )

    assert result.success, f"SM30 lookup failed: {result.error}"
    assert result.view_name == "V_T005"
    assert result.view_type == "flat"
    assert len(result.columns) > 0
    assert result.row_count > 0
    assert len(result.rows) > 0

    # V_T005 should have country-related columns
    print(f"V_T005: {result.row_count} rows, columns: {result.columns}")
    print(f"First row: {result.rows[0].values}")


@pytest.mark.anyio
async def test_sm30_lookup_v_t002_languages(sap_mcp_client: ClientSession) -> None:
    """
    Test sap_sm30_lookup with V_T002 (Languages) - another flat table view.
    """
    login = await call_tool_typed(sap_mcp_client, "sap_login", {}, LoginResult)
    assert login.success, f"Login failed: {login.error}"

    result = await call_tool_typed(
        sap_mcp_client,
        "sap_sm30_lookup",
        {"view_name": "V_T002"},
        SM30ViewResult,
    )

    assert result.success, f"SM30 lookup failed: {result.error}"
    assert result.view_type == "flat"
    assert result.row_count > 0
    print(f"V_T002: {result.row_count} rows, columns: {result.columns}")


@pytest.mark.anyio
async def test_sm30_lookup_not_found(sap_mcp_client: ClientSession) -> None:
    """
    Test sap_sm30_lookup with a non-existent view.
    """
    login = await call_tool_typed(sap_mcp_client, "sap_login", {}, LoginResult)
    assert login.success, f"Login failed: {login.error}"

    result = await call_tool_typed(
        sap_mcp_client,
        "sap_sm30_lookup",
        {"view_name": "ZZZNOTEXIST99"},
        SM30ViewResult,
    )

    assert not result.success, "Expected failure for non-existent view"
    assert "not found" in result.error.lower() or "existiert nicht" in result.error.lower()
    print(f"Not found error: {result.error}")


@pytest.mark.anyio
async def test_sm30_lookup_output_file(
    sap_mcp_client: ClientSession,
    tmp_path: pytest.TempPathFactory,
) -> None:
    """
    Test sap_sm30_lookup with output_file parameter.
    """
    from sapwebguimcp.models.sm30_models import SM30FileSummary

    login = await call_tool_typed(sap_mcp_client, "sap_login", {}, LoginResult)
    assert login.success, f"Login failed: {login.error}"

    output_file = str(tmp_path / "sm30_v_t005.json")

    result = await call_tool_typed(
        sap_mcp_client,
        "sap_sm30_lookup",
        {"view_name": "V_T005", "output_file": output_file},
        SM30FileSummary,
    )

    assert result.success, f"SM30 lookup failed: {result.error}"
    assert result.output_file
    assert result.row_count > 0
    assert len(result.sample_rows) > 0
    print(f"Output file: {result.output_file}, rows: {result.row_count}")
```

**Expected outcome:** Integration tests pass when run against a real SAP system.

**Run command:**
```bash
cd C:/github/sapwebgui.mcp
pytest unittests/test_sm30_integration.py -v -s --no-header 2>&1 | tail -30
```

---

### Task 10: Handle Unsupported View Types Gracefully

**This task is already incorporated into Tasks 5 and 7, but verifying coverage here.**

**Covered scenarios:**

1. **SM34 redirect:** `_check_sm34_redirect()` in `sm30_tools.py` checks status bar and snapshot for SM34 mentions. `detect_sm30_view_type()` in `sm30_parser.py` also detects SM34 in snapshots.

2. **Hierarchical views:** If no `columnheader` elements are found in the snapshot (indicating a non-grid layout), `detect_sm30_view_type()` returns `"unsupported"`.

3. **View not found:** `_check_view_not_found()` checks both status bar messages (DE/EN) and page title to detect if we're still on the initial screen.

4. **Error messages returned:** All unsupported cases return `SM30ViewResult.failure(...)` with `view_type="unsupported"` and a human-readable error message.

**Verification:** The parser unit tests in Task 6 include test cases for all three scenarios (`test_sm34_redirect`, `test_view_not_found`, `test_no_grid_is_unsupported`).

---

## Summary of Files

### New Files (6)
| File | Purpose |
|---|---|
| `src/sapwebguimcp/models/sm30_models.py` | Pydantic models: SM30Row, SM30ViewResult, SM30FileSummary |
| `src/sapwebguimcp/parsers/sm30_parser.py` | Parser: dynamic column/row extraction from ARIA snapshots |
| `src/sapwebguimcp/tools/sm30_tools.py` | Tool: register_sm30_tools with sap_sm30_lookup |
| `unittests/test_sm30_exploration.py` | Exploration tests to capture DE/EN snapshots |
| `unittests/test_sm30_parser.py` | Parser unit tests (offline, no SAP needed) |
| `unittests/test_sm30_integration.py` | Integration tests against real SAP |

### Modified Files (3)
| File | Change |
|---|---|
| `src/sapwebguimcp/models/__init__.py` | Add SM30 model imports and __all__ entries |
| `src/sapwebguimcp/tools/__init__.py` | Add register_sm30_tools import and __all__ entry |
| `src/sapwebguimcp/server.py` | Add register_sm30_tools import and registration call |
| `src/sapwebguimcp/lang.py` | Add SM30 language constants |

### Branch
`feat/sm30-tool`
