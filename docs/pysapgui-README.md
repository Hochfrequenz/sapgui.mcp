...
**pysapgui** gives you typed, IDE-friendly access to SAP GUI for Windows.
...
Robot Framework keywords. pysapgui wraps each SAP GUI element type with its
...
See the `examples/pysapgui/` directory for complete runnable scripts.

## Architecture

pysapgui wraps the SAP GUI Scripting COM API as a hierarchy of typed Python classes:
...
git clone https://github.com/Hochfrequenz/pysapgui.git
cd pysapgui
pip install -e ".[dev]"
