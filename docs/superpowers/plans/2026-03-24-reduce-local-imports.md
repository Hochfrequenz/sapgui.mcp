...
In `src/sapwebguimcp/backend/protocol.py`, find the `SapNavigation` protocol class (the first sub-protocol that `SapUiBackend` composes). Add a `backend_type` property:
...
In `src/sapwebguimcp/backend/desktop/__init__.py`, add to the `DesktopBackend` class:
...
In `src/sapwebguimcp/backend/webgui/backend.py`, add to the `WebGuiBackend` class:
...
Replace `src/sapwebguimcp/tools/_backend_utils.py` contents with:
...
python -m pylint src/sapwebguimcp/tools/_backend_utils.py src/sapwebguimcp/backend/protocol.py
python -m mypy --strict src/sapwebguimcp/backend/protocol.py src/sapwebguimcp/tools/_backend_utils.py
```

- [ ] **Step 7: Format and commit**

```bash
python -m isort src/sapwebguimcp/backend/protocol.py src/sapwebguimcp/backend/desktop/__init__.py src/sapwebguimcp/backend/webgui/backend.py src/sapwebguimcp/tools/_backend_utils.py unittests/desktop/test_desktop_backend.py
python -m black src/sapwebguimcp/backend/protocol.py src/sapwebguimcp/backend/desktop/__init__.py src/sapwebguimcp/backend/webgui/backend.py src/sapwebguimcp/tools/_backend_utils.py unittests/desktop/test_desktop_backend.py
...
**Files:**
- Create: `src/sapwebguimcp/backend/desktop/_landscape.py`
- Modify: `src/sapwebguimcp/tools/sap_list_connections_impl.py`
- Modify: `src/sapwebguimcp/backend/desktop/__init__.py`
- Modify: `unittests/test_sap_list_connections.py` (if it imports from impl)

- [ ] **Step 1: Create `_landscape.py`**

Create `src/sapwebguimcp/backend/desktop/_landscape.py` with `_find_landscape_path()` and `_parse_landscape_xml()` moved from `tools/sap_list_connections_impl.py`. These are lines 22-60 of that file. Keep the exact same code and docstrings.
...
python -m mypy --strict src/sapwebguimcp/backend/desktop/_landscape.py src/sapwebguimcp/tools/sap_list_connections_impl.py
```

- [ ] **Step 5: Format and commit**

```bash
python -m isort src/sapwebguimcp/backend/desktop/_landscape.py src/sapwebguimcp/tools/sap_list_connections_impl.py src/sapwebguimcp/backend/desktop/__init__.py
python -m black src/sapwebguimcp/backend/desktop/_landscape.py src/sapwebguimcp/tools/sap_list_connections_impl.py src/sapwebguimcp/backend/desktop/__init__.py
...
python -m mypy --strict src/sapwebguimcp/backend/manager.py
```

- [ ] **Step 3: Format and commit**

```bash
python -m isort src/sapwebguimcp/backend/manager.py && python -m black src/sapwebguimcp/backend/manager.py
...
Move `CheckActivateResult`, `ClosePopupResult`, `DropdownFillResult`, `FillFormResult`, `FormFieldsResult` to the top-level runtime import section (add to existing `from sapwebguimcp.models.sap_results import (...)` block). Keep only `GuiSession` in TYPE_CHECKING (sapsucker, platform-dependent).

Add `from sapwebguimcp.backend.protocol import CheckActivateResult` as a new top-level import.
...
- `from sapwebguimcp.models.config import get_settings` — add to top-level imports
- `import time` — add to stdlib imports at top
- `from sapwebguimcp.models.base import PopupButton, PopupType` — add `PopupButton, PopupType` to the existing `from sapwebguimcp.models.base import PopupInfo, ToolResult` line
...
```bash
python -m pytest unittests/desktop/test_desktop_backend.py -v
python -m mypy --strict src/sapwebguimcp/backend/desktop/__init__.py
python -m pylint src/sapwebguimcp/backend/desktop/__init__.py
```

- [ ] **Step 5: Format and commit**

```bash
python -m isort src/sapwebguimcp/backend/desktop/__init__.py && python -m black src/sapwebguimcp/backend/desktop/__init__.py
...
- Modify: `src/sapwebguimcp/backend/desktop/_discovery.py`
...
- `from sapwebguimcp.models.sap_results import SapFieldType` (used at line ~1059)
- `from sapwebguimcp.backend.webgui.browser import get_browser_manager` (used at line ~1416)
...
- `quick_report_tools.py`: promote `from sapwebguimcp.backend.manager import get_backend` (1 lazy import)
...
In `backend/desktop/_discovery.py`, find the lazy `from sapsucker.components.grid import GuiGridView` and `from sapwebguimcp.backend.desktop._element_finder import _flatten` imports. Move both to top-level: `_flatten` as a normal import, `GuiGridView` via `try/except ImportError` block.
...
python -m pylint src/sapwebguimcp
python -m mypy --strict src/sapwebguimcp
```

- [ ] **Step 5: Format and commit**

```bash
python -m isort src/sapwebguimcp && python -m black src/sapwebguimcp
...
```bash
grep -rn "pylint: disable=import-outside-toplevel" src/sapwebguimcp/ | wc -l
```
...
python -m pylint src/sapwebguimcp
python -m mypy --strict src/sapwebguimcp
python -m isort --check src/sapwebguimcp unittests
python -m black --check src/sapwebguimcp unittests
python -m pytest unittests/ -v -k "not integration" --ignore=unittests/webgui 2>&1 | tail -5
```
...
```bash
grep -rn "pylint: disable=import-outside-toplevel" src/sapwebguimcp/ | sed 's/:.*//' | sort | uniq -c | sort -rn
```
